...
- **课表性能与可读性**
  - 优化大课表窗口拉伸重绘策略，减少卡顿。
  - 提升 compact 模式课表行高与课程块高度。

- **待办卡片布局升级**
  - 桌面模式保持网格卡片；compact 模式切换为横向单列条目卡片。
  - 卡片右上角操作区（Pin + 紧急度）采用相对定位，显示更稳定。
  - 大/小窗口卡片内容区支持纵向滚动，标签或内容较多时可下滚查看。
  - 待办卡片布局参数对齐课程列表风格，宽度随容器自适应。

- **置顶待办窗口尺寸体系修复**
  - 修复 Pin 窗口“保存尺寸与实际显示尺寸不一致”的问题（统一 DIP 尺寸口径）。
  - 新增设置入口“调整置顶待办窗口大小”，支持预览、确认并持久化尺寸。
  - 调整尺寸页文案布局，提升小尺寸窗口下可读性。

- **设置页布局优化**
  - 设置页改为三列结构并稳定对齐。
  - compact 模式下自动清理列间边距，避免与第一列错位。
