...
- **待办**：待办列表与表单（标题、内容、截止日期、优先级、标签、完成状态）。
  - 支持在待办卡片上点击 **Pin** 打开“置顶小窗”，用于快速查看与勾选完成。
  - 置顶小窗支持 Tab（同一待办重复 Pin 会聚焦已有 Tab），并支持跨窗口拖拽 **拆分/合并**。
  - 主窗口与置顶小窗之间的“完成状态”支持**双向同步**（增量更新，避免全量刷新导致卡片抖动）。
